...
async def _format_vehicles_details(session: aiohttp.ClientSession, item: dict):
    ...
    return {
        ...
        'max_atmosphering_speed': item.get('max_atmosphering_speed', 'desconhecido') + ' km/h' if item.get('max_atmosphering_speed') != 'unknown' else 'desconhecido',
        ...
        'vehicle_class': item.get('vehicle_class', 'desconhecido'),
        'pilots': pilots or [],
        'films': films or [],
    }

async def list_vehicles(params: dict):
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            tasks = [_format_vehicles_details(session, item) for item in all_results]
